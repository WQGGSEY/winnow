...
아래 최초 감사의 상태와 검증 기록은 당시 관찰을 보존한 것이다. 후속 수정에서는 P1/P2의 완료 node 원고 읽기와 렌더 전 재검증, P3의 파일·입력·strong receipt 결합, P4/P5의 HTML과 부록 처리, P7의 실제 evidence anchor 및 인용 메타데이터 연결, P8/P9의 증거 기반 표·그림을 구현했다. P6의 로컬 경로·설정된 식별자 검사는 추가했지만 제출 패키지 전체의 익명성 검토는 남아 있다.

P10은 connector source packet을 보존한다. H2의 P-blind 방향 생성까지 연결한 것은 아니다. P11은 후보를 바로 baseline으로 승격하지 않고 실제 preflight 실행과 구현 소스에 역할을 결합하며, MCP 제출 도구와 목표 동결 경로를 연결했다. 과학적 비교 적합성을 자동으로 증명하지는 않는다. P12는 한 동결 연구의 확증 평가 소비 기록을 strong attestation과 terminal 재검증에 결합했다. 외부 평가 실행과 다른 연구 사이의 재사용 탐지는 해결하지 않았다.

P13은 별도 Lean 이론 계약과 실제 커널 검사·재실행 모듈을 구현했다. 공리 감사의 선언명 위조를 재현하고 차단했다. 현재 지원은 고정 Lean `Init`에 한정되며 경험적 terminal과 구분한 최종 경로 연결은 남아 있다. P14는 다섯 학회의 고정 2026 main 프로필에 대한 공식 템플릿 exporter를 추가했고 실제 PDF 컴파일을 확인했다. 이는 선택한 향후 회차의 제출 준비 완료를 뜻하지 않는다. P15는 원고·실제 증거 값에 접근하는 두 개의 격리된 검토 호출과 응답·실행 기록 재검증 모듈을 구현했다. 아직 실제 신규 논문에 대한 검토를 실행하지 않았다.

공식 패키지 export, 과학적 검토, 이론 증거 모듈을 최종 MCP·supervisor 완료 판정에 연결하는 작업이 남아 있다. 현재 HTML preview integrity 판정을 논문 제출 성공으로 사용하면 안 된다. H1/H2/H4/H5/H6/H7/H8과 실제 문제를 사용한 전체 실행도 완료되지 않았다. 따라서 추가 문제 15개와 이 세션의 성공 조건을 모두 해결했다고 선언하지 않는다.

이 상태의 전체 테스트는 `venv/bin/python -m pytest -q`에서 1018 passed, 20 subtests passed였다. 공식 프로필은 wheel 빌드 후 저장소 밖에서 다섯 개가 로드됨을 확인했다. 연구 실험이나 신규 논문 생성은 이 검증에 포함되지 않는다.

검증 문제의 최신 입력 제약은 [협력 게임 입력 계약](cooperative-symmetry-problem.md)에 기록했다. 기존 학습기·체크포인트·정책·해결 보고서를 제공하지 않고 게임 I/O와 시뮬레이터만 제공한다. 학습 코드 작성부터 하네스의 평가 대상이다.
