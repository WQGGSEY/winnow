...
def _system_prompt(allowed_domains: list[str], baseline_md: str) -> str:
    base = (
        ...
        "## PROPOSE_DATASET type-specific schema (this matters — bad specs are "
        "rejected by the deterministic materializer, not by the LLM)\n"
        "* raw_data | benchmark | model_weights: spec.source MUST be one of "
        "  hf://<org>/<repo>, file:///abs/path, or absolute /path. Bare names "
        "  like 'ImageNet' or http(s) URLs are rejected.\n"
        "* factor_set | custom: spec.source MUST be file:///abs/path or absolute "
        "  /path. The built-in fetcher does NOT download URLs. If you only "
        "  have a public-URL source (Kenneth French Library, AQR daily, Quandl, "
        "  …), either ASK the user to download it locally first, or pivot to "
        "  type=synthetic. Never emit a factor_set with a bare label / http URL "
        "  as source — it will fail and waste a round.\n"
        "* synthetic: spec MUST include a synthetic_recipe object with keys: "
        "  shape (tabular | time_series | quant_factor | scalar), rows (int >= 1), "
        "  seed (int), and — REQUIRED for tabular/time_series/quant_factor — "
        "  a columns array. Each column is "
        "  {name, distribution, params{...}} where distribution is one of "
        "  normal | bernoulli | uniform | poisson | constant. Example: "
        "  {\"id\":\"synth_x\",\"type\":\"synthetic\",\"role\":\"evaluation\","
        "  \"synthetic_recipe\":{\"shape\":\"tabular\",\"rows\":1000,\"seed\":42,"
        "  \"columns\":[{\"name\":\"x\",\"distribution\":\"normal\","
        "  \"params\":{\"mu\":0,\"sigma\":1}},{\"name\":\"y\",\"distribution\":"
        "  \"bernoulli\",\"params\":{\"p\":0.3}}]}}.\n"
        "When a PROPOSE_DATASET returns status=needs_user_input you have a "
        "clear instruction — do NOT retry the same spec; either ASK the user "
        "or pivot to synthetic with a fully-specified recipe."
    )
    if allowed_domains:
        base += (
