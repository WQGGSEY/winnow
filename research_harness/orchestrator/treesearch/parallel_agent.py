...
def _build_far_framing_successors(
    parent: dict[str, Any],
    selected_domains: list[str],
    *,
    parent_depth: int,
    max_depth: int,
    forbid_parent_approach: bool = False,
) -> list[dict[str, Any]]:
    """ADR 0009 (B1): mint one far-framing successor per HARNESS-selected distant
    domain D_i. Each successor's mission is to MAP the parent claim into D_i and
    solve it there — analytical decomposition within the home manifold yields
    only patchwork; forced distant transfer is what can reach a genuinely new
    construction. The domains are chosen by deterministic top-k policy over the
    frozen taxonomy (caller), never authored by the construction. For a
    negatively-resolved parent, forbid-the-familiar bans reusing the parent's
    approach so the successor cannot re-try the failed basin."""
    from copy import deepcopy

    from research_harness.schemas.validator import validate_named_schema

    if parent_depth >= max_depth or not selected_domains:
        return []

    parent_claim = (parent.get("claim_contract") or {}).get("claim_under_test", "")
    children: list[dict[str, Any]] = []
    for index, domain in enumerate(selected_domains, start=1):
        child = deepcopy(parent)
        child["id"] = f"{parent['id']}_far{index:02d}_{domain}"
        child["status"] = "ready"
        child["stage"] = "experimentation"
        child["parent"] = parent["id"]
        child["domain"] = domain
        ...
        child["lineage"]["introduced_assumptions"] = [
            f"Far-framing: map the parent claim into the {domain} domain and solve it there."
        ]
        child["lineage"]["far_framing_domain"] = domain
        child["claim_contract"]["claim_under_test"] = (
            f"Map the parent problem into the {domain} domain and solve it there, "
            f"then map the solution back. Parent claim: {parent_claim}"
        )
        child["claim_contract"]["success_criteria"] = [
            f"State the mapping into {domain} explicitly and what it buys.",
            *parent["claim_contract"]["success_criteria"],
        ]
        child["claim_contract"]["disproof_conditions"] = [
            f"The {domain} mapping adds nothing the home-manifold framing did not already give.",
            *parent["claim_contract"]["disproof_conditions"],
        ]
        if forbid_parent_approach and parent_claim:
            child["claim_contract"]["forbidden_approaches"] = [
                f"Do not reuse the parent's approach/method (it was negatively resolved): {parent_claim}"
            ]
        child["failure_retrieval"]["query_tags"] = sorted(
            set([*parent["failure_retrieval"]["query_tags"], "far_framing", domain])
        )
        child["outputs"] = {"artifacts": [], "verdict": None}
        validate_named_schema("node", child)
        children.append(child)
...
def _build_synthesis_node(
    parent: dict[str, Any], far_children: list[dict[str, Any]]
) -> dict[str, Any] | None:
    """ADR 0009 (B2): mint a synthesis node that recombines the partial mappings
    of the completed far-framing siblings. X lives in the recombination of
    recombinations, not a single far node — but the synthesis earns NO novelty
    credit for being a synthesis: it re-enters the SAME critic / reduction /
    falsifier / construct-adversary gates as any node. Additive recombination
    (merely y+z) is pinned as a disproof condition so the construct-adversary
    must treat a summing 'synthesis' as a pass-but-wrong world (breakable).
    Inputs are the harness's record of which siblings completed (alpha2), not an
    LLM choice. Returns None if fewer than two siblings to recombine."""
    from copy import deepcopy

    from research_harness.schemas.validator import validate_named_schema

    if len(far_children) < 2:
        return None
    sib_ids = sorted(c["id"] for c in far_children)
    domains = sorted({c.get("domain", "") for c in far_children if c.get("domain")})

    child = deepcopy(parent)
    child["id"] = f"{parent['id']}_synth"
    child["type"] = "synthesis"
    child["status"] = "ready"
    child["stage"] = "experimentation"
    child["parent"] = parent["id"]
    child["domain"] = parent.get("domain") or child.get("domain") or ""
    child["lineage"]["inherited_assumptions"] = [
        *parent["lineage"]["inherited_assumptions"],
        *parent["lineage"]["introduced_assumptions"],
    ]
    child["lineage"]["introduced_assumptions"] = [
        f"Synthesis: recombine the partial mappings from far-framing siblings {sib_ids} "
        f"(domains {domains})."
    ]
    child["lineage"]["synthesis_inputs"] = sib_ids
    child["lineage"].pop("far_framing_domain", None)  # a synthesis is not a far node
    child["claim_contract"]["claim_under_test"] = (
        f"Recombine the partial mappings from far-framing siblings {sib_ids} into a "
        "single construction that none of them yields alone."
    )
    child["claim_contract"]["success_criteria"] = [
        "Name what the recombination yields that no single sibling mapping did.",
        *parent["claim_contract"]["success_criteria"],
    ]
    child["claim_contract"]["disproof_conditions"] = [
        "Merely additively combining the siblings (y+z) without a genuinely new "
        "construction passes the measurement yet the frozen question's answer is NO "
        "— additive recombination is a pass-but-wrong world the construct-adversary "
        "must search; a summing 'synthesis' is broken there.",
        *parent["claim_contract"]["disproof_conditions"],
    ]
    child["failure_retrieval"]["query_tags"] = sorted(
        set([*parent["failure_retrieval"]["query_tags"], "synthesis"])
    )
    child["outputs"] = {"artifacts": [], "verdict": None}
    validate_named_schema("node", child)
    return child
