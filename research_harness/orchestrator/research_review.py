...
def review_research_packet(repo: Path, directory: Path, packet: dict[str, Any], *, purpose: str) -> dict[str, Any]:
    ...
    if development:
        instructions += (
            ...
            'Verify each binding value_kind: support_count requires positive sample support or validity gates; measurement requires only finiteness. '
            'Signed effects and outcome fractions must be measurement, so zero or negative results remain interpretable with valid support. '
            'Reject a measurement label that bypasses a required support count or validity gate, and reject a support_count label that gates on a positive scientific effect. '
