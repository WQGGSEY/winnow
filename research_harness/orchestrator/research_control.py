...
def plan_research_work(repo: Path, thread: Path, *, reconsider_reason: str = '', transport=None) -> dict[str, Any]:
    ...
    packet = {
        ...
        'observation_binding_contract': 'For empirical work, supply experiment_plan.observation_bindings mapping every required_observations name to artifact_path (a declared metrics file), json_pointer, producer (source location), and value_kind (support_count for positive support or validity gates; measurement for any finite outcome, including zero/negative effects). Bind existing emitted values explicitly; never guess aliases or silently aggregate observations.',
...
Choose from:
- analysis: a distinct semantic or source question answerable from existing records that changes which experiment to run. It may recompute summaries from saved observations or inspect saved predictions and source. Fitting or modifying a model, including a capacity control on saved training data, is new empirical work even if called analysis-only; select diagnostic_experiment or competence for that work. Do not make an analysis prediction depend on a new fit or rollout. Existing findings are fallible but should not be repeatedly re-audited without a specific unresolved distinction. source_mode=acquire only when primary material actually needs acquisition.
...
ONE ResearchWork is a scheduling unit, not a requirement that a study-design amendment authorize only one future development trial. An explicit protocol-scope rejection is a planning constraint: resolve the cited authorization problem before selecting another empirical work. Changing a test's label, removing a simulator call, reusing saved data or changing its budget does not itself establish permission. If a different proposed test is already permitted, identify the visible operative permission and explain why the rejected restriction does not apply; do not assert compliance with unseen text. Otherwise select protocol_revision. When consumed one-off permissions repeatedly block informative follow-ups, propose a prospective bounded adaptive development procedure: identify the restrictions it explicitly replaces, permitted development data and changes, a finite aggregate trial/compute budget, evidence-dependent continuation and stopping rules, and eventual candidate/comparator freeze before independent confirmation. Each later empirical work still requires its own pre-execution source, observation bindings and review. Existing restrictions remain active until that amendment is approved. Do not repeatedly spend a protocol revision on an equivalent one-off solely because work selection is singular. Preserve the original goal, final evaluation separation, success bar and disclosure of all attempts; development adaptation cannot qualify a result or permit outcome-selected confirmation retries.
