...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": "While baseline qualification is pending, write and execute ONE baseline implementation or bounded development experiment. Prefer supplying experiment_plan without node: the harness derives operational metadata, input snapshot, and the identical claim contract automatically. An explicit node remains subject to exact contract validation. source_files may use from_path (absolute existing thread file), sha256, and optional replacements:[{old,new}] instead of content; every old text must match exactly once, and the original file is preserved. To revise a saved dispatch, use its absolute request_path plus updates (path segments and new value), without retransmitting unchanged source code or editing authoritative dispatch files. Draft files may be edited in get_research_state.source_workspace and registered with from_path/sha256. To change node identity, update experiment_plan.node_id; update node.id as well only if the saved request supplied an explicit node. Inline objects work for initial requests. Use a new node ID when changing a previously executed plan; completed receipts are immutable. The harness binds the operator input snapshot, enforces compute limits and runs LocalRunner. For a baseline role, use exactly one baseline_evidence_requirement and report only that baseline key. The harness derives the dispatch role from the plan; no separate role input is needed. For selected diagnostic_experiment, competence, comparison or replication work, use mandatory_baselines=[] and baseline_evidence_requirements=[] to record an unqualified development measurement. An intervention/control pilot may emit both arms and paired effects under metrics, without a qualifying baseline key. This does not qualify either method or support a formal claim; source/protocol review and heldout separation still apply. For the same already permitted diagnostic, independent pre-execution approval also records an authoritative source-binding receipt before launch; no separate binding-only amendment is needed unless the protocol explicitly requires a separate transaction. Diagnostic receipts cannot support a scientific claim. This is execution evidence, not scientific approval. Implement methods yourself after retrieving primary sources.",
        "inputSchema": {
            "type": "object",
            ...
            "properties": {
                ...
                "role": {"type": "string", "enum": ["current_best_known", "naive", "random_or_null", "diagnostic"], "description": "Optional consistency check. The harness derives the role from the plan's baseline requirements; empty qualifying-baseline requirements require selected diagnostic_experiment, competence, comparison or replication work."},
