...
def _validate_contract(contract: dict[str, Any]) -> None:
    ...
    if contract["imports"] != ["Init"]:
        raise TheoryEvidenceError(
            "only the pinned Lean Init trust base may be imported"
        )
...
def _run(lean: Path, source: Path, declaration: str) -> tuple[str, list[str]]:
    ...
    target = re.escape(declaration)
    match = re.search(rf"'{target}' depends on axioms:\s*\[(.*?)\]", log)
    no_axioms = re.search(rf"'{target}' does not depend on any axioms", log)
    if not match and no_axioms:
        return log, []
    if not match:
...
def _init_digest(lean: Path) -> str:
    result = subprocess.run(
        [str(lean), "--print-libdir"], capture_output=True, text=True, check=True
    )
    init = Path(result.stdout.strip()).resolve() / "Init.olean"
    if not init.is_file():
        raise TheoryEvidenceError("Lean Init.olean trust base is unavailable")
    return _digest_file(init)
...
def check_theory_evidence(
    ...
    *,
    ...
    if any(line and not line[0].isspace() for line in body.splitlines()):
        raise TheoryEvidenceError("proof body may not escape the generated by block")
    ...
    log, axioms = _run(lean, source, contract["declaration_name"])
    ...
    receipt = {
        ...
        "init_olean_sha256": _init_digest(lean),
...
def verify_theory_evidence(
    ...
    body = proof.read_text(encoding="utf-8")
    if (
        _FORBIDDEN.search(body)
        or any(line and not line[0].isspace() for line in body.splitlines())
        or source.read_text(encoding="utf-8") != _source(contract, body)
    ):
    ...
    replay_log, axioms = _run(lean, source, contract["declaration_name"])
