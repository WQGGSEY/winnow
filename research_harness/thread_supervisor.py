    ...
    if work_path.exists():
        try:
            ...
            summary["research_work"] = {
                ...
                "next_tool_to_call": work.get("next_tool_to_call"),
...
def build_resume_prompt(repo: Path, tid: str, cycle: int) -> str:
    ...
    lines = [
        ...
        *needs_block,
        ...
        "     read_research_artifact로 소스·영수증·문헌 파일을 읽고 전체 파일 sha256을 확인해. 셸 실행 도구는 제공되지 않는다.",
        "     문헌 후보는 update_baseline_sources로 제출하고, 코드는 design_experiment_template의 source_files.content 또는 from_path/sha256/replacements로 작성·수정해.",
        "     준비 도구의 정적 진단으로 명백한 구현 오류를 고쳐. 새 학습이나 시뮬레이션은 smoke, feasibility, analysis-only라고 불러도 연구 실행이므로 등록·예산·검토를 거쳐야 한다.",
        "     execute_baseline_preflight 또는 execute_node_experiment로 실행하고 실패한 시도도 보존해. 결과를 보고 조건을 바꾸는 개발 탐색은 그 범위를 허용하는 프로토콜과 별도 work로 진행해.",
        ...
        "  3. 현재 research_work의 후속 절차를 마친 뒤 상태가 claim 진행을 허용하면 get_next_admissible_node를 호출해. 권위 있는 blind active node가 있으면",
        ...
        "       falsifier가 fail하면 achieved=true 불가. 확인 실험의 실패 기록을 보존하고, 같은 holdout에 맞춰 수정하거나 다시 추첨·재측정하지 마. 후속 연구는 등록된 규칙이 허용하는 별도 연구 절차를 따라야 해.",
...
def resume_known_research_step(repo: Path, tid: str, model: str, active_child: dict) -> int | None:
    ...
    if work.get('status') == 'completed' and tool == 'plan_research_work':
        ...
        arguments = {'thread_id': tid, 'work_id': work['work_id']}
        response_name = 'supervisor_analysis.jsonl'
        message = '선택된 기존 자료 분석을 MCP 분석 담당자에게 직접 요청합니다.'
