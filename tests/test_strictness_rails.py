...
def test_rejected_dossier_does_not_persist_claim_handoff(tmp_path, monkeypatch):
    class _Accepted:
        ok = True
    ...
    tid = "t_rejected_dossier"
    thread_dir = tmp_path / tid
    monkeypatch.setattr(M, "_thread_dir", lambda _tid: thread_dir)
    monkeypatch.setattr(M, "validate_claim_contract", lambda **_kwargs: _Accepted())
    monkeypatch.setattr(
        M,
        "validate_decision_rule_for_capability_claim",
        lambda **_kwargs: _Accepted(),
    )
    monkeypatch.setattr(
        M,
    ...
    (thread_dir / "grilling").mkdir(parents=True)
    (thread_dir / "grilling" / "grilling_session.json").write_text(
        json.dumps({"extracted": {"claim_under_test": "Original problem"}}),
        encoding="utf-8",
    )
    (thread_dir / "market").mkdir()
    (thread_dir / "market" / "baseline_dossier_candidate.yaml").write_text(
        "selected:\n  one_paragraph_reason: Top-ranked search result\n",
        encoding="utf-8",
    )

    result = M.handle_design_initial_claim_contract(
        {
            "thread_id": tid,
            "claim_under_test": "An active intervention improves utility by 5%.",
            "mandatory_baselines": ["incumbent", "naive", "random"],
            "success_criteria": ["utility >= 0.05"],
            "disproof_conditions": ["utility < 0.05"],
    ...
    assert not (
        thread_dir / "production" / "intake_to_claim_dialog.json"
    ).exists()
...
def test_downclamp_signals_synthetic_only_envelope(tmp_path, monkeypatch):
    ...
    reasons = M._compute_ac_downclamp_signals(tid)
...
def _legacy_state_with_closed_children():
    nodes = [
    ...
    nodes.extend(
        {"id": f"n_promoted_c{index}", "parent": "n_promoted", "status": "pruned"}
...
def test_empty_legacy_frontier_routes_to_blind_reorientation(tmp_path, monkeypatch):
    ...
    state = _legacy_state_with_closed_children()
