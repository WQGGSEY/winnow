...
# --- Auto-resolver escalation wiring ------------------------------------ #


def test_auto_resolver_refusal_enqueues_operator_prompt(tmp_path, monkeypatch):
    """When _maybe_auto_dispatch refuses (here: tool not in allowlist),
    it also escalates an operator prompt so the operator can intervene."""
    monkeypatch.setattr(M, "_thread_dir", lambda tid: tmp_path / "runs" / "threads" / tid)
    tid = "t_esc"
    response = {
    ...
    out = M._maybe_auto_dispatch(tid, response)
    assert out["auto_resolved"]["dispatched"] is False
    assert "operator_prompt" in out
    assert out["operator_prompt"]["event_id"].startswith("opr_esc_")
    # Queue file now has the escalation entry.
    pending = list_pending(tmp_path / "runs" / "threads" / tid)
    assert len(pending) == 1
    assert pending[0]["kind"] == "auto_resolver_escalation"
    assert pending[0]["source_rail"] == "rail_5_must_revise_root"


def test_escalation_event_id_stable_across_identical_refusals(tmp_path, monkeypatch):
    """Same suggestion + same refusal_reason produces the same event_id ->
    operator sees one prompt, not a spam of identical ones."""
    monkeypatch.setattr(M, "_thread_dir", lambda tid: tmp_path / "runs" / "threads" / tid)
    tid = "t_idem"
    response = {
        "auto_action_suggestion": {
    ...
    M._maybe_auto_dispatch(tid, dict(response))
    M._maybe_auto_dispatch(tid, dict(response))
    pending = list_pending(tmp_path / "runs" / "threads" / tid)
    # Folded state collapses both enqueues under the same stable event_id.
    assert len(pending) == 1
